    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑试卷选题</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑试卷选题</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">试卷选题信息</h3>
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 级联表的字段 -->
                                ...
                                <div class="form-group col-md-6 aaaaaa examquestion">
                                    ...
                                    <div class="form-group col-md-6 examquestion examquestionOptions">
                                        <label>选项</label>
                                        <input style="width: 450px" id="examquestionOptions" name="examquestionOptions" class="form-control" readonly>
                                    </div>
                                    <div class="form-group col-md-6 examquestion">
                                        <label>正确答案</label>
                                        <input style="width: 450px" id="examquestionAnswer" name="examquestionAnswer" class="form-control" readonly>
                                    </div>
                                    <div class="form-group col-md-6 examquestion">
                                        <label>答案解析</label>
                                        <input style="width: 450px" id="examquestionAnalysis" name="examquestionAnalysis" class="form-control" readonly>
                                    </div>
                            <!-- 当前表的字段 -->
                                    <input id="updateId" name="id" type="hidden">
                                <input id="exampaperId" name="exampaperId" type="hidden">
                                <input id="examquestionId" name="examquestionId" type="hidden">
                                    <div class="form-group col-md-6 exampapertopicNumberDiv">
                                        <label>试题分数</label>
                                        <input style="width: 450px" id="exampapertopicNumber" name="exampapertopicNumber" class="form-control"
                                               onchange="integerChickValue(this)"  placeholder="试题分数">
                                    </div>
...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            if(window.sessionStorage.getItem('role') != '试卷表'){//当前登录用户不为这个
                if($("#exampaperId") !=null){
                    ...
                    if(exampaperId == null || exampaperId =='' || exampaperId == 'null'){
                        layui.layer.msg("试卷不能为空", {time: 2000,icon: 5});
                        return;
                    }
                }
            }
            if(window.sessionStorage.getItem('role') != '试题表'){//当前登录用户不为这个
                if($("#examquestionId") !=null){
                    ...
                    if(examquestionId == null || examquestionId =='' || examquestionId == 'null'){
                        layui.layer.msg("试题不能为空", {time: 2000,icon: 5});
                        return;
                    }
                }
            }
            ...
            httpJson("exampapertopic/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addexampapertopic');
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                ...
                exampapertopicNumber: "required",
            },
            messages: {
                exampaperId: "试卷不能为空",
                examquestionId: "试题不能为空",
                exampapertopicNumber: "试题分数不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addexampapertopic = window.sessionStorage.getItem("addexampapertopic");
        if (addexampapertopic != null && addexampapertopic != "" && addexampapertopic != "null") {
            //注册表单验证
            $(validform());
            ...
            window.sessionStorage.removeItem('updateId');
            http("exampapertopic/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#exampapertopicNumber").val(ruleForm.exampapertopicNumber);
    ...
    function examquestionDataBind(){
        ...
        if(!(ruleForm.examquestionTypes==1 || ruleForm.examquestionTypes==2)){//不为单选题和多选题的话删除选项div框
            $(".examquestionOptions").remove();
        }
    }
