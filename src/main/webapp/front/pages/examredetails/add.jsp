...
<!-- 唯一uuid -->
    ...
    <script>
        ...
        layui.use(['layer', 'element', 'carousel', 'http', 'jquery', 'form', 'upload', 'laydate','tinymce'], function() {
            ...
            localStorage.setItem("goUtl","./pages/examredetails/list.jsp")
           ...
           // 上传文件
            ...
            //修改
            if(http.getParam('examredetailsId')){
                http.request(`examredetails/info/${http.getParam('examredetailsId')}`, 'get', {}, function(data) {
            ...
            form.on('submit(thisSubmit)', function (data) {
                ...
                if(http.getParam('examredetailsId')){
                    data["id"]= http.getParam('examredetailsId')
                }
                // 提交数据
                http.requestJson(`examredetails/${http.getParam("examredetailsId")?"update":"add"}`, 'post', data, function (res) {
                    if(res.code == 0){
                        layer.msg('提交成功', {
                            time: 2000,
                            icon: 6
                        }, function () {
                            vue.jump("../examredetails/list.jsp");
                                http.remove('examredetailsId')
                            // back();
                        });
                    }else{
