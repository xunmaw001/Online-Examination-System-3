...
<!-- 标题 -->
 ...
 <div class="proleft fl" style="width: 20%;">
     ...
     <ul class="ul1">
         <li v-for="(item,index) in centerMenu" v-bind:key="index" :class="item.url=='../examredetails/list.jsp'?'on':''" @click="jump(item.url)">
             <b></b>{{item.name}}
         </li>
     </ul>
 </div>        <!-- 个人中心菜单 -->
...
<!--                <button @click="jump('../examredetails/add.jsp')" class="layui-btn layui-btn-lg btn-theme sub_backgroundColor" :style='{"padding":"0 15px","boxShadow":"0 0 8px rgba(0,0,0,0)","margin":"0 0 0 10px","borderColor":"#409EFF","color":"#333","borderRadius":"4px","borderWidth":"0","width":"auto","fontSize":"14px","borderStyle":"solid","height":"40px"}'>
                    <i v-if="true" class="layui-icon">&#xe654;</i> 添加答题详情表信息
                </button>-->
            </div>
        ...
        <!-- 级联表 -->
                        ...
                        <td>{{item.examredetailsUuidNumber}}</td>
                        <td>{{item.examredetailsMyanswer}}</td>
                        <td>{{item.examredetailsMyscore}}</td>
                        <td style="width: 100px;">
                        <!--
                            <button @click="jump('../examredetails/add.jsp?examredetailsId='+item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                 修改
                            </button>
                        -->
                            ...
                            <button v-if="item.examredetailsTypes==1 && false" @click="wuyong(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                无用按钮
                            </button>
                        </td>
                    </tr>
    ...
    var vue = new Vue({
        ...
        methods: {
            ...
            wuyong(id) {
                ...
                layui.http.requestJson(`examredetails/update`, 'post', {
                    id:id,
//                    examredetailsTypes:1,
                }, function (res) {
                    if(res.code == 0){
            ...
            deleteData(data){
                ...
                // 删除信息
                layui.http.requestJson(`examredetails/delete`, 'post', [data.id], (res) => {
                    if(res.code==0){
                        layer.msg('删除成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery', 'laydate', 'tinymce'], function() {
        ...
        localStorage.setItem("goUtl","./pages/examredetails/list2.jsp")
