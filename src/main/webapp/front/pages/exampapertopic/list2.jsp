...
<!-- 标题 -->
 ...
 <div class="proleft fl" style="width: 20%;">
     ...
     <ul class="ul1">
         <li v-for="(item,index) in centerMenu" v-bind:key="index" :class="item.url=='../exampapertopic/list.jsp'?'on':''" @click="jump(item.url)">
             <b></b>{{item.name}}
         </li>
     </ul>
 </div>        <!-- 个人中心菜单 -->
...
<!--                <button @click="jump('../exampapertopic/add.jsp')" class="layui-btn layui-btn-lg btn-theme sub_backgroundColor" :style='{"padding":"0 15px","boxShadow":"0 0 8px rgba(0,0,0,0)","margin":"0 0 0 10px","borderColor":"#409EFF","color":"#333","borderRadius":"4px","borderWidth":"0","width":"auto","fontSize":"14px","borderStyle":"solid","height":"40px"}'>
                    <i v-if="true" class="layui-icon">&#xe654;</i> 添加试卷选题信息
                </button>-->
            </div>
        ...
        <!-- 级联表 -->
                        ...
                        <td>{{item.exampaperValue}}</td>
                        <td>{{item.examquestionName}}</td>
                        <td>{{item.examquestionValue}}</td>
                        <td>{{item.exampapertopicNumber}}</td>
                        <td style="width: 100px;">
                        <!--
                            <button @click="jump('../exampapertopic/add.jsp?exampapertopicId='+item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                 修改
                            </button>
                        -->
                            ...
                            <button v-if="item.exampapertopicTypes==1 && false" @click="wuyong(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                无用按钮
                            </button>
                        </td>
                    </tr>
    ...
    var vue = new Vue({
        ...
        methods: {
            ...
            wuyong(id) {
                ...
                layui.http.requestJson(`exampapertopic/update`, 'post', {
                    id:id,
//                    exampapertopicTypes:1,
                }, function (res) {
                    if(res.code == 0){
            ...
            deleteData(data){
                ...
                // 删除信息
                layui.http.requestJson(`exampapertopic/delete`, 'post', [data.id], (res) => {
                    if(res.code==0){
                        layer.msg('删除成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery', 'laydate', 'tinymce'], function() {
        ...
        localStorage.setItem("goUtl","./pages/exampapertopic/list2.jsp")
        ...
        // var id = http.getParam('id');
            ...
            function pageList() {
                // 获取列表数据
                http.request('exampapertopic/list', 'get', vue.searchForm, function (res) {
                    ...
                    laypage.render({
                        ...
                        jump: function (obj, first) {
                            ...
                            //首次不执行
                            if (!first) {
                                http.request('exampapertopic/list', 'get', vue.searchForm, function (res1) {
