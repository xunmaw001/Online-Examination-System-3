                ...
                <div class="layui-form-item main_borderColor" :style='{"padding":"10px","boxShadow":"0 0 0px rgba(255,0,0,.8)","margin":"0 0 10px 0","backgroundColor":"rgba(255, 255, 255, 0)","borderRadius":"1px","borderWidth":"0 0 1px 0","borderStyle":"solid"}'>
                    ...
                    <div class="layui-input-block">
                        <input class="layui-input main_borderColor" :style='{"padding":"0 12px","boxShadow":"0 0 0px rgba(160, 67, 26, 1)","backgroundColor":"#fff","color":"rgba(135, 206, 250, 1)","borderRadius":"4px","textAlign":"left","borderWidth":"1px","fontSize":"15px","borderStyle":"solid","height":"40px"}'
                           v-model="detail.exampapertopicNumber" lay-verify="integer|required" type="text" :readonly="ro.exampapertopicNumber" name="exampapertopicNumber" id="exampapertopicNumber" autocomplete="off">
                    </div>
                </div>
                <div class="layui-form-item main_borderColor" :style='{"padding":"10px","boxShadow":"0 0 0px rgba(255,0,0,.8)","margin":"0 0 10px 0","backgroundColor":"rgba(255, 255, 255, 0)","borderRadius":"1px","borderWidth":"0 0 1px 0","borderStyle":"solid"}'>
    ...
    <script>
        ...
        var vue = new Vue({
            ...
            data: {
                ...
                exampaperList:[],//级联表列表 试卷
                exampaper: {},//级联表单个 试卷
                examquestionList:[],//级联表列表 试题
                examquestion: {},//级联表单个 试题
        ...
        layui.use(['layer', 'element', 'carousel', 'http', 'jquery', 'form', 'upload', 'laydate','tinymce'], function() {
            ...
            localStorage.setItem("goUtl","./pages/exampapertopic/list.jsp")
           ...
           // 上传文件
            ...
            //修改
            if(http.getParam('exampapertopicId')){
                http.request(`exampapertopic/info/${http.getParam('exampapertopicId')}`, 'get', {}, function(data) {
            ...
            form.on('submit(thisSubmit)', function (data) {
                ...
                if(http.getParam('exampapertopicId')){
                    data["id"]= http.getParam('exampapertopicId')
                }
                // 提交数据
                http.requestJson(`exampapertopic/${http.getParam("exampapertopicId")?"update":"add"}`, 'post', data, function (res) {
                    if(res.code == 0){
                        layer.msg('提交成功', {
                            time: 2000,
                            icon: 6
                        }, function () {
                            vue.jump("../exampapertopic/list.jsp");
                                http.remove('exampapertopicId')
                            // back();
                        });
                    }else{
