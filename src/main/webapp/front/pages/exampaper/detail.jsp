            ...
            <div class="row">
                <div class="col-lg-8">
                    <div class="blog-details-wrapper">
                        <div class="thumb">
                            <img :src="baseUrl+detail.exampaperPhoto" alt="image">
                        </div>
                        <div class="blog-details-header">
                            <h3 class="blog-details-title">{{title}}</h3>
                            <ul class="post-meta">
                                <li v-if="detail.exampaperDate">考试时长(分钟)：
                                    {{detail.exampaperDate}}
                                </li>
                                <li v-if="detail.exampaperMyscore">试卷总分数：
                                    {{detail.exampaperMyscore}}
                                </li>
                                <li v-if="detail.kemuTypes">科目：
                                    {{detail.kemuValue}}
                                </li>
                                <li v-if="detail.exampaperTypes">试卷状态：
                                    {{detail.exampaperValue}}
                                </li>
                                <li v-if="detail.zujuanTypes">组卷方式：
                                    {{detail.zujuanValue}}
                                </li>
                            </ul>
                        ...
                        <!-- <div style="padding: 20px;border: 0px solid #f2f2f2;margin-top: 20px;display: flex">
                                 <div class="num-picker">
                                     <button @click="exampaperYuyue()" style="height:auto;" :style='{"padding":"0 10px","boxShadow":"0 0 0px rgba(255,0,0,.3)","margin":"0 5px","borderColor":"rgba(0,0,0,.3)","backgroundColor":"rgba(23, 124, 176, 1)","color":"rgba(255, 255, 255, 1)","borderRadius":"6px","borderWidth":"0","width":"auto","lineHeight":"40px","fontSize":"16px","borderStyle":"solid"}' type="button" class="layui-btn btn-submit">
                                         立即预约
                                     </button>
                                 </div>
                             </div>-->
                ...
                <div class="col-lg-4" v-if="exampaperRecommendList">
                    <div class="sidebar">
                        <div class="widget">
                            <h3 class="widget-title">系统推荐</h3>
                            <ul class="small-post-list">
                                <li class="small-post-single" v-for="(item,index) in exampaperRecommendList"
                                    v-bind:key="index"
                                    @click="jump('../exampaper/detail.jsp?id='+item.id)">
                                    <div class="thumb"><img width="65px" :src="baseUrl+item.exampaperPhoto"
                                                            alt="image">
                                    </div>
                                    <div class="content">
                                        <a class="main_color" class="date">{{item.exampaperName}}</a>
                                        <h6 v-if="item.exampaperNewMoney" class="post-title"><a
                                                style="color: red">{{item.exampaperNewMoney}}</a>RMB</h6>
                                    </div>
                                </li>
                            </ul>
                        </div>
    ...
    <script>
        ...
        var vue = new Vue({
            ...
            methods: {
                //分享
                share(){
                    let element = createElement("http://localhost:8080/wangluozaixiankaoshi/front/index.jsp?url="+"./pages/exampaper/detail.jsp?id="+this.id);
                    element.select();
                ...
                exampaperYuyue(){
                    ...
                    // localStorage.setItem('exampaperId', _this.detail.id);
                    // _this.jump("../exampaperOrder/add.jsp");

                    let data={
                        exampaperId:_this.detail.id,
                        yonghuId:localStorage.getItem("userid"),
                        exampaperOrderYesnoTypes:1,
                    }
                    // 提交数据
                    layui.http.requestJson('exampaperOrder/add', 'post', data, function (res) {
                        if(res.code == 0) {
                            layui.layer.msg('预约成功', {
                                time: 2000,
                                icon: 6
                            }, function () {
                                _this.jump("../exampaperOrder/list.jsp");
                            });
                        }else{
        ...
        layui.use(['layer', 'form', 'element', 'carousel', 'http', 'jquery', 'laypage', 'util'], function () {
            ...
            localStorage.setItem("goUtl","./pages/exampaper/detail.jsp?id="+ http.getParam('id'))
            ...
            http.request(`${vue.detailTable}/detail/` + id, 'get', {}, function (res) {
                // 详情信息
                vue.detail = res.data;
                vue.title = vue.detail.exampaperName;
                // 轮播图片
                vue.swiperList = vue.detail.exampaperPhoto ? vue.detail.exampaperPhoto.split(",") : [];
                // 轮播图
                vue.$nextTick(() => {
                    carousel.render({
                        elem: '#swiper',
            ...
            http.request(`exampaper/list`, 'get', {
                page: 1,
                limit: 5,
                exampaperTypes: vue.detail.exampaperTypes,
                exampaperDelete: 1,
                kemuTypes: vue.detail.kemuTypes,
                zujuanTypes: vue.detail.zujuanTypes,
            }, function (res) {
                vue.exampaperRecommendList = res.data.list;
            });
