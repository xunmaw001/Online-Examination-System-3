...
<!-- 唯一uuid -->
                <div class="layui-form-item main_borderColor" :style='{"padding":"10px","boxShadow":"0 0 0px rgba(255,0,0,.8)","margin":"0 0 10px 0","backgroundColor":"rgba(255, 255, 255, 0)","borderRadius":"1px","borderWidth":"0 0 1px 0","borderStyle":"solid"}'>
                    ...
                    <div class="layui-input-block">
                        <input class="layui-input main_borderColor" :style='{"padding":"0 12px","boxShadow":"0 0 0px rgba(160, 67, 26, 1)","backgroundColor":"#fff","color":"rgba(135, 206, 250, 1)","borderRadius":"4px","textAlign":"left","borderWidth":"1px","fontSize":"15px","borderStyle":"solid","height":"40px"}'
                               v-model="detail.examrecordUuidNumber" lay-verify="required" type="text" :readonly="ro.examrecordUuidNumber" name="examrecordUuidNumber" id="examrecordUuidNumber" autocomplete="off">
                    </div>
                </div>
                <!-- 级联表的表id -->
                ...
                <div class="layui-form-item main_borderColor" :style='{"padding":"10px","boxShadow":"0 0 0px rgba(255,0,0,.8)","margin":"0 0 10px 0","backgroundColor":"rgba(255, 255, 255, 0)","borderRadius":"1px","borderWidth":"0 0 1px 0","borderStyle":"solid"}'>
                    ...
                    <div class="layui-input-block">
                        <input class="layui-input main_borderColor" :style='{"padding":"0 12px","boxShadow":"0 0 0px rgba(160, 67, 26, 1)","backgroundColor":"#fff","color":"rgba(135, 206, 250, 1)","borderRadius":"4px","textAlign":"left","borderWidth":"1px","fontSize":"15px","borderStyle":"solid","height":"40px"}'
                           v-model="detail.totalScore" lay-verify="integer|required" type="text" :readonly="ro.totalScore" name="totalScore" id="totalScore" autocomplete="off">
                    </div>
                </div>
                <div class="layui-form-item main_borderColor" :style='{"padding":"10px","boxShadow":"0 0 0px rgba(255,0,0,.8)","margin":"0 0 10px 0","backgroundColor":"rgba(255, 255, 255, 0)","borderRadius":"1px","borderWidth":"0 0 1px 0","borderStyle":"solid"}'>
    ...
    <script>
        ...
        var vue = new Vue({
            ...
            data: {
                ...
                detail: {
                    examrecordUuidNumber: new Date().getTime(),//数字
                    yonghuId: '',
                    exampaperId: '',
                    totalScore: '',
                    insertTime: '',
                ...
                yonghuList:[],//级联表列表 考试用户
                yonghu: {},//级联表单个 考试用户
                exampaperList:[],//级联表列表 所属试卷id（外键）
                exampaper: {},//级联表单个 所属试卷id（外键）
        ...
        layui.use(['layer', 'element', 'carousel', 'http', 'jquery', 'form', 'upload', 'laydate','tinymce'], function() {
            ...
            localStorage.setItem("goUtl","./pages/examrecord/list.jsp")
           ...
           // 上传文件
            ...
            //修改
            if(http.getParam('examrecordId')){
                http.request(`examrecord/info/${http.getParam('examrecordId')}`, 'get', {}, function(data) {
            ...
            form.on('submit(thisSubmit)', function (data) {
                ...
                if(http.getParam('examrecordId')){
                    data["id"]= http.getParam('examrecordId')
                }
                // 提交数据
                http.requestJson(`examrecord/${http.getParam("examrecordId")?"update":"add"}`, 'post', data, function (res) {
                    if(res.code == 0){
                        layer.msg('提交成功', {
                            time: 2000,
                            icon: 6
                        }, function () {
                            vue.jump("../examrecord/list.jsp");
                                http.remove('examrecordId')
                            // back();
                        });
                    }else{
